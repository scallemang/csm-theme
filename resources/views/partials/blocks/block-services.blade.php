...
<section
  class="block-services 
    @if( !$width )services--rounded @else services--full @endif
    @if( 'bg-none' == $background['class'] )my-5 @else py-5 @endif  {{ $style }} {{ $background['class'] }} @if( $background['overlay'] )overlay-{{ $background['overlay']['color'] }}@endif" 
